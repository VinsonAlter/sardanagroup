...
<head>
    ...
    <title>Sardana Group | Dealer Resmi Mitsubishi Medan</title>
...
<body>
    <?php  
       require_once("./mitsubishi/component/Header.php");
    ?>
    <section>
        <div class="mt-header">
            <?php readfile('./component/Carousel.php')?>
        </div>
        <div id="PC" class="mt15">
            <?php readfile("./mitsubishi/component/PC.php")?>
        </div>
        <div id="LCV" class="mt-5">
            <?php readfile("./mitsubishi/component/LCV.php")?>
        </div>
        <div id="CV" class="mt-5">
            <?php readfile("./fuso/component/CV.php")?>
        </div>
        <section id="lokasiDealer" class="mt15">
           <?php readfile("./component/LokasiDealer.php")?>
        </section>
    </section>
    <section id="berita" class="d-flex align-items-center mt15">
        <div class="container">
    ...
    <div class="pt-3">
        <?php readfile("./component/Footer.php")?>
    </div>
        ...
        $(document).ready(() => {
            if(hash === '#bookingservice'){
                ...
                if(issm){
                    ...
                    $('.overlayBg').load('https://www.sardanagroup.co.id/page/bookingservice/', () => $('.overlayBg .container').click(e => e.stopPropagation()))
                }
                else{
                    $('.btMenu').click()
                ...
                if(issm){
                    $('body').toggleClass('noScroll')
                    $('.menuOverlay').toggleClass('sideBarActive')
                    $('.overlayBg').load('https://www.sardanagroup.co.id/page/suarapelanggan/', () => $('.overlayBg .container').click(e => e.stopPropagation()))
                }
                else{
                    $('.btMenu').click()
