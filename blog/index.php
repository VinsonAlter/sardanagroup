...
<head>
    ...
    <title>Blog Mitsubishi | Sardana Berlian</title>
...
<body>
      ...
      <div class="swiper-container swiper-container-blog">
        <div class="swiper-wrapper">
          <?php
            ...
            $result = $koneksi->query("SELECT * FROM news WHERE kategori = 'SG' ORDER BY id DESC LIMIT 0,5");
          ...
          ?>            
            <div class="swiper-slide">
              ...
              <a class="btn btn-outline-light selengkapnya" name="btnSelengkapnya" href="detail/<?=$row['seo_link']?>"><i class="fas fa-plus mr-2"></i>SELENGKAPNYA</a>
            </div>
          <?php            
            endwhile;
          ?>
    ...
    <div class="kategori text-black mt-3" id="kategori">
      <div class="container">
        <div class="row mmc-medium text-right">
          <div class="col-2">
    ...
    <div class="container mt-5" id="hasil">    
      <!-- Isi disini -->     
      <div class="row">
        <?php
          // $result = $koneksi->query("SELECT * FROM NEWS WHERE kategori = 'SG' AND (tipe = 'promo' OR tipe = 'event' OR tipe = 'hype')");
          $result = $koneksi->query("SELECT * FROM news WHERE kategori = 'SG' AND (tipe = 'PROMO' OR tipe = 'EVENT' OR tipe = 'HYPE') ORDER BY id DESC LIMIT 0,9");
          while ($row = $result->fetch_array()) :
            $date = strtotime($row["tanggalpublish"]);
            $date = date("d-m-Y", $date);
        ?>
        <div class="col-12 col-sm-6 col-lg-4">
          <div class="card" style="border: 1px solid gray; overflow: hidden">
            <img src="https://sardanagroup.co.id/HP/production/images/news/<?=$row['Paths']?>" class="card-img-top" alt="Blog Mitsubishi Sardana Berlian">
            <div class="card-body">
